...
\caption{\label{tab:data_cleaning_display}Table containing Unclean News Data}
\centering
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{kable}\NormalTok{(}\KeywordTok{head}\NormalTok{(news), }\DataTypeTok{format =} \StringTok{"latex"}\NormalTok{, }\DataTypeTok{booktabs =}\NormalTok{ T,}
        \DataTypeTok{caption=}\KeywordTok{paste}\NormalTok{(}\StringTok{"Table containing Clean News Data"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \KeywordTok{c}\NormalTok{(}\StringTok{"striped"}\NormalTok{,}\StringTok{"hold_position"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{news_body =}\StringTok{ }\NormalTok{news}\OperatorTok{$}\NormalTok{body}
\NormalTok{tokenized_words =}\StringTok{ }\KeywordTok{tokenize_word_stems}\NormalTok{(news_body, }\DataTypeTok{stopwords =}\NormalTok{ stopwords}\OperatorTok{::}\KeywordTok{stopwords}\NormalTok{(}\StringTok{"en"}\NormalTok{))}
\CommentTok{#tokenized_words}

\CommentTok{# print(paste("Number of tokenised words: ",length(tokenized_words)))}
\CommentTok{# corpus_words = Corpus(VectorSource(tokenized_words))}
\CommentTok{# term_matrix = DocumentTermMatrix(corpus_words,}
\CommentTok{#                    control=list(}
\CommentTok{#                        tokenize=newBigramTokenizer,}
\CommentTok{#       Show in New WindowClear OutputExpand/Collapse Output}

\CommentTok{# term_matrix}
\CommentTok{# inspect(term_matrix)}
\end{Highlighting}
\end{Shaded}
